...
const DropdownContent = styled(motion.div)({
    position: "absolute", backgroundColor: "#fff", color: '#222',
    borderRadius: '10px', zIndex: 10,
    minWidth: "200px", boxShadow: "0px 4px 6px rgba(0, 0, 0, 0.1)"
});
const StyledButton = styled(Button)(({ active }) => ({
    textTransform: "none", display: "flex",
    justifyContent: "space-between", alignItems: "center",
    width: "100%", background: active ? '#e8eaf6' : "inherit",
    color: active ? '#3f50b5' : 'inherit', padding: "8px 12px",
    borderRadius: "50px", "&:hover": { color: "#000" }
}));
...
const DropdownMenu = ({ items }) => {
    ...
    return items?.map((item, index) => (
        <Dropdown key={index} onMouseEnter={() => setOpenIndex(index)} onMouseLeave={() => setOpenIndex(null)}>
            <StyledButton sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }} href={item.route || "#"} active={location.pathname === item.route}>
                    ...
                    <DropdownContent variants={variants} initial="hidden" animate="visible" exit="exit" sx={{ mt: 1 }} transition={{ duration: 0.3 }}>
                        {item.subMenu.map((subItem, subIndex) => (
                            <Dropdown key={subIndex} onMouseEnter={() => setNestedIndex(subIndex)} onMouseLeave={() => setNestedIndex(null)}>
                                <StyledButton href={subItem.route || "#"} active={location.pathname === subItem.route} >
                                    ...
                                    {nestedIndex === subIndex && subItem.subMenu && (
                                        <DropdownContent variants={nestedVariants} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.3 }} sx={{ left: "100%", top: 0 }}>
                                            {subItem.subMenu.map((nestedItem, nestedIndex) => (
                                                <StyledButton key={nestedIndex} href={nestedItem.route || "#"} active={location.pathname === nestedItem.route} >
                                                    <motion.div whileHover={{ x: 5 }} transition={{ type: 'spring', stiffness: 300 }}>
